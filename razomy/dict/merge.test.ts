...
describe('dictionary', () => {
  describe('merge', () => {
    ...
    it('merges two non-overlapping dictionaries', () => {
      expect(dict.merge([{ a: 1 }, { b: 2 }])).toEqual({ a: 1, b: 2 });
    });

    it('merges with later entries overwriting earlier ones for duplicate keys', () => {
      expect(dict.merge([{ a: 1 }, { a: 2, b: 3 }])).toEqual({ a: 2, b: 3 });
    });

    it('merges three dictionaries with mixed types', () => {
      expect(dict.merge([{ x: 'hello' }, { y: 'world' }, { z: true }])).toEqual({
        x: 'hello',
        y: 'world',
        z: true,
      });
