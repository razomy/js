...
describe('dictionary', () => {
  describe('map', () => {
    ...
    it('maps each value using the callback function', () => {
      expect(map({ a: 1, b: 2 }, (v) => v * 2)).toEqual({ a: 2, b: 4 });
    });

    it('maps values with a string transformation', () => {
      expect(map({ x: 'hello', y: 'world' }, (v) => v.toUpperCase())).toEqual({
        x: 'HELLO',
        y: 'WORLD',
      });
    });
    ...
    it('passes the correct arguments to the callback (value, key)', () => {
      expect(map({ a: 'hi' }, (v, k) => `${k}:${v}`)).toEqual({ a: 'a:hi' });
    });

    it('passes value and key for every entry', () => {
      const args: Array<[number, string]> = [];

      map({ foo: 1, bar: 2, baz: 3 }, (value, key) => {
        args.push([value, key]);
        return value;
      });
    ...
    it('preserves the original keys in the resulting object', () => {
      const result = map({ name: 'Alice', age: 'thirty' }, (v) => v.length);
    ...
    it('can transform values to a different type', () => {
      const result = map({ a: 1, b: 2, c: 3 }, (v) => v > 1);
    ...
    it('works correctly with a single-entry object', () => {
      expect(map({ only: 42 }, (v) => v + 8)).toEqual({ only: 50 });
    });

    // 7. Does not mutate the original object
    it('does not mutate the original object', () => {
      const original = { a: 1, b: 2 };
      const result = map(original, (v) => v * 10);
    ...
    it('works correctly with objects as values', () => {
      ...
      const result = map(input, (v) => v.score);
