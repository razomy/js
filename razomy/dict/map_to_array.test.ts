...
describe('object', () => {
  describe('mapToArray', () => {
    ...
    it('maps object entries to an array of transformed values', () => {
      expect(dict.mapToArray({ a: 1, b: 2 }, (k, v) => `${k}${v}`)).toEqual(['a1', 'b2']);
    });

    it('maps a single-entry object correctly', () => {
      expect(dict.mapToArray({ x: 10 }, (k, v) => v * 2)).toEqual([20]);
    });

    // 2. Empty object
    it('returns an empty array for an empty object', () => {
      expect(dict.mapToArray({}, (k, v) => v)).toEqual([]);
    });

    // 3. Callback arguments
    it('passes the correct key and value to the callback', () => {
      const args: Array<[string, number]> = [];
      dict.mapToArray({ foo: 1, bar: 2 }, (k, v) => {
        args.push([k, v]);
        return v;
      });
    ...
    it('supports returning objects from the callback', () => {
      const result = dict.mapToArray({ a: 1, b: 2 }, (k, v) => ({ key: k, value: v }));
    ...
    it('supports returning booleans from the callback', () => {
      const result = dict.mapToArray({ a: 1, b: 0, c: 3 }, (_k, v) => v > 0);
      expect(result).toEqual([true, false, true]);
    });

    it('supports returning arrays from the callback', () => {
      const result = dict.mapToArray({ a: 1, b: 2 }, (k, v) => [k, v]);
      expect(result).toEqual([
        ['a', 1],
        ['b', 2],
      ]);
    ...
    it('returns an array with the same length as the number of keys', () => {
      ...
      const result = dict.mapToArray(obj, (_k, v) => v);
      expect(result).toHaveLength(5);
    });

    // 7. Callback using only key
    it('works when callback only uses the key', () => {
      const result = dict.mapToArray({ first: 100, second: 200 }, (k) => k.toUpperCase());
      expect(result).toEqual(['FIRST', 'SECOND']);
    });

    // 8. Callback using only value
    it('works when callback only uses the value', () => {
      const result = dict.mapToArray({ a: 5, b: 10, c: 15 }, (_k, v) => v + 1);
