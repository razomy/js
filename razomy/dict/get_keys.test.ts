...
describe('object', () => {
  describe('getKeys', () => {
    ...
    it('returns the keys of an object with string values', () => {
      expect(getKeys({ name: 'Raz', id: 101 })).toEqual(['name', 'id']);
    });
    ...
    it('returns keys regardless of value types', () => {
      ...
      expect(getKeys(obj)).toEqual(['str', 'num', 'bool', 'nul', 'undef', 'arr', 'nested']);
    });
    ...
    it('does not include inherited properties', () => {
      ...
      expect(getKeys(child)).toEqual(['own']);
    });
    ...
    it('does not include symbol keys', () => {
      ...
      expect(getKeys(obj)).toEqual(['a']);
    });

    // 8. Array-like objects
    it('returns index keys for arrays', () => {
      const arr = ['x', 'y', 'z'];
      expect(getKeys(arr)).toEqual(['0', '1', '2']);
    });
    ...
    it('preserves insertion order of keys', () => {
      ...
      expect(getKeys(obj)).toEqual(['c', 'a', 'b']);
    });
  });
});
