...
describe('object', () => {
  describe('take', () => {
    ...
    it('preserves references for object and array values', () => {
      ...
      const result = dict.take(obj, ['arr', 'nested']);
    ...
    it('does not include keys that are not specified', () => {
      const result = dict.take({ a: 1, b: 2, c: 3 }, ['a'] as const);
    ...
    it('handles duplicate keys gracefully', () => {
      const obj = { a: 1, b: 2 };
      const result = dict.take(obj, ['a', 'a'] as Array<'a' | 'b'>);
      expect(result).toEqual({ a: 1 });
    });
    ...
    it('works with objects that have string values', () => {
      ...
      expect(dict.take(obj, ['firstName', 'lastName'])).toEqual({
        firstName: 'John',
        lastName: 'Doe',
      });
    });
    ...
    it('returns a new object, not the original', () => {
      const obj = { a: 1, b: 2 };
      const result = dict.take(obj, ['a', 'b']);
