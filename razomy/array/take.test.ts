...
describe('array', () => {
  describe('take', () => {
    ...
    it('takes 1 element from the beginning', () => {
      expect(array.take([1, 2, 3], 1)).toEqual([1]);
    });
    ...
    it('takes all elements when n equals array length', () => {
      expect(array.take([1, 2, 3], 3)).toEqual([1, 2, 3]);
    });
    ...
    it('returns an end array when n is negative', () => {
      expect(array.take([1, 2, 3], -1)).toEqual([1, 2]);
    });

    // 5. Empty array
    it('returns an empty array when the input array is empty', () => {
      expect(array.take([], 3)).toEqual([]);
    });
    ...
    it('does not mutate the original array', () => {
      ...
      const result = array.take(original, 2);
    ...
    it('returns a new array, not the same reference', () => {
      const original = [1, 2, 3];
      const result = array.take(original, 3);
    ...
    it('works correctly with arrays of objects', () => {
      ...
      expect(array.take(items, 2)).toEqual([{ id: 1 }, { id: 2 }]);
    });

    it('works correctly with arrays of strings', () => {
      expect(array.take(['a', 'b', 'c', 'd'], 3)).toEqual(['a', 'b', 'c']);
    });

    // 9. Single element array
    it('works correctly with a single element array', () => {
      expect(array.take([42], 1)).toEqual([42]);
      expect(array.take([42], 5)).toEqual([42]);
      expect(array.take([42], 0)).toEqual([]);
    });
  });
});
