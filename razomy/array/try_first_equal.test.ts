...
describe('array', () => {
  describe('tryFirstEqual', () => {
    ...
    it('returns the first element from array2 that exists in array1', () => {
      expect(array.tryFirstEqual([1, 2, 3], [4, 2, 3])).toBe(2);
    });

    it('returns the first match based on array2 order, not array1 order', () => {
      expect(array.tryFirstEqual([10, 20, 30], [30, 20, 10])).toBe(30);
    });

    it('returns null if no elements match', () => {
      expect(array.tryFirstEqual(['a', 'b'], ['c', 'd'])).toBeNull();
    });

    // 2. Empty arrays
    it('returns null if array1 is empty', () => {
      expect(array.tryFirstEqual([], [1, 2, 3])).toBeNull();
    });
    ...
    it('matches by reference for objects (strict equality)', () => {
      ...
      const obj2 = { a: 1 };
      const obj3 = obj1;
      expect(array.tryFirstEqual([obj1], [obj2])).toBeNull();
      expect(array.tryFirstEqual([obj1], [obj3])).toBe(obj1);
    });

    // 5. Multiple matches - returns the first from array2
    it('returns the first match from array2 when multiple matches exist', () => {
      expect(array.tryFirstEqual([1, 2, 3], [3, 2, 1])).toBe(3);
    });

    // 6. Duplicate elements
    it('handles duplicates in array2 correctly', () => {
      expect(array.tryFirstEqual([2], [1, 2, 2, 3])).toBe(2);
    });

    it('handles duplicates in array1 correctly', () => {
      expect(array.tryFirstEqual([2, 2, 2], [1, 3, 2])).toBe(2);
    });

    // 7. Various types
    it('works with strings', () => {
      expect(array.tryFirstEqual(['foo', 'bar'], ['baz', 'bar', 'foo'])).toBe('bar');
    });

    it('works with boolean values', () => {
      expect(array.tryFirstEqual([true], [false, true])).toBe(true);
    });

    it('works with null and undefined values in arrays', () => {
      expect(array.tryFirstEqual([null, undefined], [undefined, null])).toBe(undefined);
    });

    // 8. Short-circuiting behavior
    it('returns on the first match without checking the rest of array2', () => {
      expect(array.tryFirstEqual([1, 2, 3, 4, 5], [6, 7, 3, 4, 5])).toBe(3);
    });
  });
});
