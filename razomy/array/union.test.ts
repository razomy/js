...
describe('array', () => {
  describe('union', () => {
    ...
    it('returns unique values from multiple arrays', () => {
      expect(array.union(['a'], ['b'], ['a'])).toEqual(['a', 'b']);
    });
    ...
    it('preserves the order of first occurrence', () => {
      expect(array.union([3, 1, 2], [2, 4, 1])).toEqual([3, 1, 2, 4]);
    });

    // 3. Empty arrays
    it('returns an empty array when no arrays are provided', () => {
      expect(array.union()).toEqual([]);
    });
    ...
    it('returns unique values from a single array', () => {
      expect(array.union([1, 2, 2, 3, 3, 3])).toEqual([1, 2, 3]);
    });
    ...
    it('treats different object references as distinct values', () => {
      const obj1 = { a: 1 };
      const obj2 = { a: 1 };
      expect(array.union([obj1], [obj2])).toEqual([obj1, obj2]);
    });

    it('deduplicates the same object reference across arrays', () => {
      const obj = { a: 1 };
      expect(array.union([obj], [obj])).toEqual([obj]);
    });

    // 8. Mixed types
    it('works with mixed types', () => {
      expect(array.union([1, 'a'], ['a', 2])).toEqual([1, 'a', 2]);
    });

    // 9. Many arrays
    it('handles many arrays', () => {
      expect(array.union([1], [2], [3], [4], [5])).toEqual([1, 2, 3, 4, 5]);
    });

    // 10. Boolean and falsy values
    it('handles falsy values correctly', () => {
      expect(array.union([0, false, ''], [null, undefined, 0])).toEqual([0, false, '', null, undefined]);
    });
  });
});
