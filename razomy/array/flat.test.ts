...
describe('array', () => {
  describe('flat', () => {
    ...
    it('flattens mixed elements and nested arrays by one level', () => {
      expect(array.flat([1, 2, [3, [4]]])).toEqual([1, 2, 3, [4]]);
    });

    it('flattens arrays with string elements', () => {
      expect(array.flat([['a'], 'b'])).toEqual(['a', 'b']);
    });
    ...
    it('returns a shallow copy when there are no nested arrays', () => {
      expect(array.flat([1, 2, 3])).toEqual([1, 2, 3]);
    });

    // 4. Only one level of flattening
    it('only flattens one level deep', () => {
      expect(array.flat([[[1, 2]], [[3, 4]]])).toEqual([
        [1, 2],
        [3, 4],
      ]);
    });
    ...
    it('does not flatten deeply nested arrays', () => {
      expect(array.flat([[[['deep']]]])).toEqual([[['deep']]]);
    });

    // 5. Mixed types
    it('handles mixed nested and non-nested elements', () => {
      expect(array.flat([1, [2, 3], 4, [5]])).toEqual([1, 2, 3, 4, 5]);
    });

    // 6. Single element arrays
    it('flattens a single nested array', () => {
      expect(array.flat([[1]])).toEqual([1]);
    });
    ...
    it('works correctly with arrays of objects', () => {
      ...
      const obj2 = { b: 2 };
      const obj3 = { c: 3 };
      expect(array.flat<any>([[obj1, obj2], [obj3]])).toEqual([obj1, obj2, obj3]);
    });

    it('preserves reference equality for objects', () => {
      const obj = { key: 'value' };
      const result = array.flat([[obj]]);
    ...
    it('handles larger arrays', () => {
      ...
      expect(array.flat(input)).toEqual(expected);
    });
  });
});
