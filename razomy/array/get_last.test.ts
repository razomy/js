...
describe('array', () => {
  describe('getLast', () => {
    ...
    it('returns the second to last element with offset 1', () => {
      expect(array.getLast(['a', 'b', 'c'], 1)).toBe('b');
    });

    it('returns the first element when offset equals array length minus 1', () => {
      expect(array.getLast([1, 2, 3], 2)).toBe(1);
    });

    // 2. Single element array
    it('returns the only element of a single-element array', () => {
      expect(array.getLast([42])).toBe(42);
    });

    it('throws an error for a single-element array with offset 1', () => {
      expect(() => array.getLast([42], 1)).toThrow('Element at offset 1 does not exist.');
    });

    // 3. Empty array
    it('throws an error for an empty array', () => {
      expect(() => array.getLast([])).toThrow('Element at offset 0 does not exist.');
    });

    it('throws an error for an empty array with an offset', () => {
      expect(() => array.getLast([], 5)).toThrow('Element at offset 5 does not exist.');
    });

    // 4. Out of bounds offset
    it('throws an error when offset is larger than array length', () => {
      expect(() => array.getLast([1, 2, 3], 10)).toThrow('Element at offset 10 does not exist.');
    });

    it('throws an error when offset equals array length', () => {
      expect(() => array.getLast([1, 2, 3], 3)).toThrow('Element at offset 3 does not exist.');
    });

    // 5. Default offset
    it('uses offset 0 by default', () => {
      expect(array.getLast([10, 20, 30])).toBe(30);
    });

    // 6. Complex data types
    it('works correctly with arrays of objects', () => {
      const items = [{ id: 1 }, { id: 2 }, { id: 3 }];
      expect(array.getLast(items)).toEqual({ id: 3 });
      expect(array.getLast(items, 1)).toEqual({ id: 2 });
    });

    it('works correctly with arrays of strings', () => {
      expect(array.getLast(['foo', 'bar', 'baz'])).toBe('baz');
    });

    // 7. Edge case: undefined values in array
    it('throws an error when the element at the position is undefined', () => {
      expect(() => array.getLast([1, undefined, 3], 1)).toThrow('Element at offset 1 does not exist.');
    });

    it('throws an error for an array of all undefined values', () => {
      expect(() => array.getLast([undefined, undefined])).toThrow('Element at offset 0 does not exist.');
    });

    // 8. Offset 0 explicitly
    it('returns the last element when offset is explicitly 0', () => {
      expect(array.getLast([5, 10, 15], 0)).toBe(15);
    });
  });
});
