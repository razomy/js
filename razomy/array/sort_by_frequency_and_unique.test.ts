...
describe('array', () => {
  describe('sortByFrequencyAndUnique', () => {
    ...
    it('sorts by frequency in descending order and returns unique values', () => {
      expect(array.sortByFrequencyAndUnique([1, 2, 2, 2, 1])).toEqual([2, 1]);
    });

    it('sorts strings by frequency in descending order', () => {
      expect(array.sortByFrequencyAndUnique(['a', 'a', 'a', 'b', 'b', 'c'])).toEqual(['a', 'b', 'c']);
    });

    it('sorts numbers by frequency in descending order', () => {
      expect(array.sortByFrequencyAndUnique([10, 20, 10, 10, 20])).toEqual([10, 20]);
    });

    // 2. Empty array
    it('returns an empty array when given an empty array', () => {
      expect(array.sortByFrequencyAndUnique([])).toEqual([]);
    });

    // 3. Single element
    it('returns single element array when given a single element', () => {
      expect(array.sortByFrequencyAndUnique([42])).toEqual([42]);
    });

    // 4. All elements are the same
    it('returns a single element when all elements are identical', () => {
      expect(array.sortByFrequencyAndUnique([5, 5, 5, 5])).toEqual([5]);
    });

    // 5. All elements have the same frequency
    it('returns unique elements when all have equal frequency', () => {
      const result = array.sortByFrequencyAndUnique([1, 2, 3]);
    ...
    it('handles elements with tied frequencies', () => {
      const result = array.sortByFrequencyAndUnique([1, 1, 2, 2, 3]);
    ...
    it('works with string arrays', () => {
      expect(array.sortByFrequencyAndUnique(['x', 'y', 'x', 'z', 'x', 'y'])).toEqual(['x', 'y', 'z']);
    });

    // 8. Preserves uniqueness
    it('removes duplicates from the result', () => {
      const result = array.sortByFrequencyAndUnique([3, 3, 3, 1, 1, 2]);
    ...
    it('correctly orders elements with various frequencies', () => {
      ...
      const result = array.sortByFrequencyAndUnique([1, 2, 2, 3, 3, 3, 4, 4, 4, 4]);
