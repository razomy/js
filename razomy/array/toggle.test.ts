...
describe('array', () => {
  describe('toggle', () => {
    ...
    it('removes the first occurrence of the item', () => {
      expect(toggle([1, 2, 3, 2], 2)).toEqual([1, 3, 2]);
    });

    // 3. Empty array
    it('appends the item to an empty array', () => {
      expect(toggle([], true)).toEqual([true]);
    });

    it('appends a number to an empty array', () => {
      expect(toggle([], 42)).toEqual([42]);
    });
    ...
    it('returns a new array when appending and does not mutate the original', () => {
      const original = [1, 2];
      const result = toggle(original, 3);
    ...
    it('works with object references', () => {
      ...
      expect(toggle([obj1, obj2], obj3)).toEqual([obj1, obj2, obj3]);
      expect(toggle([obj1, obj2, obj3], obj2)).toEqual([obj1, obj3]);
    });

    it('does not match objects by value, only by reference', () => {
      const obj = { id: 1 };
      const result = toggle([obj], { id: 1 });
    ...
    it('works correctly with string arrays', () => {
      expect(toggle(['hello', 'world'], 'hello')).toEqual(['world']);
      expect(toggle(['hello', 'world'], 'foo')).toEqual(['hello', 'world', 'foo']);
    });

    // 9. Boolean items
    it('works correctly with boolean arrays', () => {
      expect(toggle([true, false], true)).toEqual([false]);
      expect(toggle([false], true)).toEqual([false, true]);
    });
  });
});