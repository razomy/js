...
describe('array', () => {
  describe('map', () => {
    ...
    it('converts elements to strings', () => {
      expect(map([1, 2, 3], String)).toEqual(['1', '2', '3']);
    });

    it('uses index in the iteratee', () => {
      expect(map(['a', 'b'], (char, index) => char + index)).toEqual(['a0', 'b1']);
    });
    ...
    it('returns an array of the same length as the input', () => {
      const result = map([10, 20, 30, 40, 50], (n) => n + 1);
      expect(result).toHaveLength(5);
    });

    // 6. Single element array
    it('works with a single element array', () => {
      expect(map([42], (n) => n * 2)).toEqual([84]);
    });
    ...
    it('works correctly with arrays of objects', () => {
      ...
      expect(map(users, (u) => u.name)).toEqual(['Alice', 'Bob']);
    });

    // 8. Type transformation
    it('can change the type of elements', () => {
      const result = map([1, 2, 3], (n) => ({ value: n }));
