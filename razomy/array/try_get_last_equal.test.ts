...
describe('array', () => {
  describe('tryGetLastEqual', () => {
    ...
    it('returns the last matching pair when a match exists', () => {
      expect(tryGetLastEqual([1, 2, 3], [3, 4, 5], (a, b) => a === b)).toEqual([3, 3]);
    });

    it('returns the last matching pair with case-insensitive string comparison', () => {
      expect(tryGetLastEqual(['a', 'b'], ['A', 'B'], (a, b) => a.toUpperCase() === b)).toEqual(['b', 'B']);
    });

    it('returns null when no match is found', () => {
      expect(tryGetLastEqual([1, 2], [3, 4], (a, b) => a === b)).toBeNull();
    });

    // 2. Empty arrays
    it('returns null when the first array is empty', () => {
      expect(tryGetLastEqual([], [1, 2, 3], (a, b) => a === b)).toBeNull();
    });
    ...
    it('returns null when both arrays are empty', () => {
      expect(tryGetLastEqual([], [], (a, b) => a === b)).toBeNull();
    });
    ...
    it('works with different types for the two arrays', () => {
      const arrayA = [1, 2, 3];
      const arrayB = ['1', '2', '3'];
      const result = tryGetLastEqual(arrayA, arrayB, (a, b) => String(a) === b);
