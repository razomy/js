...
describe('array', () => {
  describe('set', () => {
    ...
    it('replaces the element at the specified index', () => {
      expect(array.set(['a', 'b', 'c'], 1, 'x')).toEqual(['a', 'x', 'c']);
    });

    it('replaces the first element', () => {
      expect(array.set([1, 2, 3], 0, 99)).toEqual([99, 2, 3]);
    });

    it('replaces the last element', () => {
      expect(array.set([1, 2, 3], 2, 99)).toEqual([1, 2, 99]);
    });

    it('replaces an element with the same value', () => {
      expect(array.set([0, 1, 0], 1, 0)).toEqual([0, 0, 0]);
    });

    // 2. Negative indices
    it('supports negative index to count from the end', () => {
      expect(array.set([1, 2, 3], -1, 99)).toEqual([1, 2, 99]);
    });

    it('supports negative index -2', () => {
      expect(array.set([1, 2, 3], -2, 99)).toEqual([1, 99, 3]);
    });

    it('supports negative index equal to -array.length (first element)', () => {
      expect(array.set([1, 2, 3], -3, 99)).toEqual([99, 2, 3]);
    });

    // 3. Out of bounds
    it('throws RangeError when index is >= array.length', () => {
      expect(() => array.set([1, 2, 3], 3, 99)).toThrow(RangeError);
    });

    it('throws RangeError when index is < -array.length', () => {
      expect(() => array.set([1, 2, 3], -4, 99)).toThrow(RangeError);
    });

    it('throws RangeError for an empty array with index 0', () => {
      expect(() => array.set([], 0, 99)).toThrow(RangeError);
    });

    // 4. Immutability
    it('returns a new array and does not mutate the original', () => {
      const original = [1, 2, 3];
      const result = array.set(original, 1, 99);
    ...
    it('works with a single element array', () => {
      expect(array.set([42], 0, 100)).toEqual([100]);
    });

    it('works with a single element array and negative index', () => {
      expect(array.set([42], -1, 100)).toEqual([100]);
    });
    ...
    it('works with arrays of objects', () => {
      ...
      expect(array.set([obj1, obj2, obj3], 1, newObj)).toEqual([obj1, newObj, obj3]);
    });
    ...
    it('handles floating point index by truncating to integer', () => {
      ...
      expect(array.set([1, 2, 3], 1.7, 99)).toEqual([1, 99, 3]);
    });
  });
});
