...
describe('array', () => {
  describe('countBy', () => {
    ...
    it('counts occurrences grouped by string length', () => {
      expect(countBy(['one', 'two', 'three'], (v) => v.length)).toEqual({ '3': 2, '5': 1 });
    });
    ...
    it('handles numeric predicate results as string keys', () => {
      const result = countBy([1, 2, 3, 4, 5, 6], (n) => n % 3);
