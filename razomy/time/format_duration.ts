...
export function formatDuration(durationInMs: number): string {
  ...
  if (duration.days && duration.days > 0) {
    return `${duration.days} day${duration.days > 1 ? 's' : ''}${duration.hours ? `, ${duration.hours} hour${duration.hours > 1 ? 's' : ''}` : ''}`;
  } else if (duration.hours && duration.hours > 0) {
    return `${duration.hours} hour${duration.hours > 1 ? 's' : ''}${duration.minutes ? `, ${duration.minutes} min` : ''}`;
