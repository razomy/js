...
describe('string', () => {
  describe('stripTags', () => {
    ...
    it('strips self-closing tags like <hr />', () => {
      expect(stripTags('Before<hr />After')).toBe('BeforeAfter');
    });

    it('strips <img> tags with attributes', () => {
      expect(stripTags('Look at this: <img src="image.png" alt="photo" />')).toBe('Look at this: ');
    });

    // 4. Tags with various attributes
    it('strips tags with multiple attributes', () => {
      expect(stripTags('<div class="container" id="main" data-value="42">Text</div>')).toBe('Text');
    });

    it('strips tags with single-quoted attributes', () => {
      expect(stripTags("<div class='test'>Content</div>")).toBe('Content');
    });

    // 5. Multiple tags and mixed content
    it('strips multiple different tags from a string', () => {
      expect(stripTags('<h1>Title</h1><p>Paragraph</p>')).toBe('TitleParagraph');
    });

    it('handles mixed text and tags', () => {
      expect(stripTags('Hello <b>bold</b> and <i>italic</i> text')).toBe('Hello bold and italic text');
    });

    // 6. Deeply nested tags
    it('strips deeply nested tags', () => {
      expect(stripTags('<div><ul><li><a href="#">Item</a></li></ul></div>')).toBe('Item');
    });

    // 7. Edge cases with special characters
    it('preserves content with special characters outside of tags', () => {
      expect(stripTags('<p>Price: $5 &amp; tax</p>')).toBe('Price: $5 &amp; tax');
    });
    ...
    it('handles greater-than signs in text that are not part of tags', () => {
      expect(stripTags('5 > 3')).toBe('5 > 3');
    });

    // 11. Comment-like structures
    it('strips HTML comments', () => {
      expect(stripTags('Hello <!-- comment --> world')).toBe('Hello  world');
    });
