...
describe('string', () => {
  describe('repeat', () => {
    ...
    it('repeats a word multiple times', () => {
      expect(string.repeat('razomy', 2)).toBe('razomyrazomy');
    });
    ...
    it('repeats strings with unicode characters', () => {
      expect(string.repeat('🚀', 3)).toBe('🚀🚀🚀');
    });

    // 6. Larger counts
    it('repeats a string a large number of times', () => {
      const result = string.repeat('ab', 100);
    ...
    it('repeats multi-word strings correctly', () => {
      expect(string.repeat('ha ', 3)).toBe('ha ha ha ');
    });
  });
});
