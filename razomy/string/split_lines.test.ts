...
describe('string', () => {
  describe('splitLines', () => {
    ...
    it('splits a string by newline (\\n) characters', () => {
      expect(string.splitLines('Line 1\nLine 2')).toEqual(['Line 1', 'Line 2']);
    });
    ...
    it('splits a string by carriage return (\\r) characters', () => {
      expect(string.splitLines('Line 1\rLine 2')).toEqual(['Line 1', 'Line 2']);
    });

    it('splits a string with mixed newline characters', () => {
      expect(string.splitLines('A\r\nB\nC')).toEqual(['A', 'B', 'C']);
    });
    ...
    it('returns an array with the original string if no newlines are present', () => {
      expect(string.splitLines('One')).toEqual(['One']);
    });

    // 3. Empty string
    it('returns an array with one empty string for an empty input', () => {
      expect(string.splitLines('')).toEqual(['']);
    });
    ...
    it('handles a string that is just \\r', () => {
      expect(string.splitLines('\r')).toEqual(['', '']);
    });
    ...
    it('splits many lines correctly', () => {
      const input = 'a\nb\nc\nd\ne\nf';
      expect(string.splitLines(input)).toEqual(['a', 'b', 'c', 'd', 'e', 'f']);
    });
