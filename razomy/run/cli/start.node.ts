...
export async function start() {
  try {
    run.validateArrayStringThrow(process.argv.slice(2), 'terminalArgs');
    ...
    const result = await runNode.resolveAndRun(filePathOrPackageName, pathWithFunctionName, params);
