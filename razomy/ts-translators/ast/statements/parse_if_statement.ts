...
export function parseIfStatement(node: IfStatement): abstracts.translators.ConditionalFlowStatement {
  ...
  const thenBranch: abstracts.translators.BranchFlowStatement = {
    ...
    type: 'if',
    pattern: tsTranslators.ast.expressions.parseExpression(node.getExpression()),
    block: tsTranslators.ast.statements.parseBlockStatement(node.getThenStatement()),
  };
  ...
  if (elseNode) {
    ...
    const elseBranch: abstracts.translators.BranchFlowStatement = {
      ...
      pattern: null,
      block: tsTranslators.ast.statements.parseBlockStatement(elseNode),
    };
