...
export function parse(node: Statement): abstracts.translators.DeclarationType {
  if (Node.isBlock(node)) {
    return tsTranslators.ast.statements.parseBlock(node);
  }

  if (Node.isIfStatement(node)) {
    return tsTranslators.ast.statements.parseCondition(node);
  }
  ...
  if (
    ...
    return tsTranslators.ast.statements.parseLoop(node);
  }

  if (Node.isReturnStatement(node)) {
    // Кастуем как any, так как в текущей онтологии ReturnExpression не входит в DeclarationType
    return tsTranslators.ast.statements.parseReturn(node) as any;
  }

  if (Node.isBreakStatement(node) || Node.isContinueStatement(node)) {
    return tsTranslators.ast.statements.parseGo(node);
  }

  if (Node.isThrowStatement(node)) {
    return tsTranslators.ast.statements.parseThrow(node);
  }
  ...
  if (Node.isVariableStatement(node)) {
    ...
    if (declarations.length > 0) {
      return tsTranslators.ast.bindings.parseVariable(declarations[0] as any);
    }
  }
  ...
  if (Node.isExpressionStatement(node)) {
    ...
    if (Node.isCallExpression(node2)) {
      //TODO: add bridge;
      return tsTranslators.ast.expressions.parseCall(node2) as any;
    }
    if (Node.isUnaryExpression(node2)) {
      //TODO: add bridge;
      return tsTranslators.ast.expressions.parseUnary(node2) as any;
    }
