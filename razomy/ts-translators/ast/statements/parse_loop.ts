...
export function parseLoop(node: Statement): abstracts.translators.LoopFlowStatement {
  ...
  if (Node.isForStatement(node)) {
    ...
    if (condNode) condition = tsTranslators.ast.expressions.parse(condNode);

    const updNode = node.getIncrementor();
    if (updNode) update = tsTranslators.ast.expressions.parse(updNode);

  } else if (Node.isWhileStatement(node)) {
    type = 'while_do';
    bodyNode = node.getStatement();
    condition = tsTranslators.ast.expressions.parse(node.getExpression());
  ...
  return {
    ...
    condition,
    update,
    block: tsTranslators.ast.statements.parseBlock(bodyNode),
  };
}
