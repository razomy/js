...
export function parse(node: Node): abstracts.translators.BindingType | abstracts.translators.ShapeBindingType {
  if (Node.isVariableDeclaration(node)) {
    return tsTranslators.ast.bindings.parseVariable(node) as abstracts.translators.VariableBinding;
  }

  if (Node.isTypeAliasDeclaration(node)) {
    return tsTranslators.ast.shapeBindings.parseAlias(node);
  }

  if (Node.isInterfaceDeclaration(node)) {
    return tsTranslators.ast.shapeBindings.parseInterface(node);
  }

  if (Node.isClassDeclaration(node)) {
    return tsTranslators.ast.shapeBindings.parseClass(node);
  }

  if (Node.isEnumDeclaration(node)) {
    return tsTranslators.ast.bindings.parseEnum(node);
  }

  if (Node.isFunctionDeclaration(node)) {
    return tsTranslators.ast.bindings.parseFunction(node);
  }
  ...
  if (Node.isExpressionStatement(node)) {
    ...
    if (Node.isBinaryExpression(expr)) {
      return {
        ...
        expression: tsTranslators.ast.expressions.parse(expr.getRight()), // This gets the value
      };
    }
