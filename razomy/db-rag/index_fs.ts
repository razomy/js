...
export async function indexFs(db, projectPath: string, lastCommitId: string, filesData: dbRag.ChunkFile[]) {
  ...
  try {
    ...
    async function flushBatch() {
      ...
      const embeddings = await Promise.all(batchData.map((b) => dbRag.getEmbedding(b.text)));
