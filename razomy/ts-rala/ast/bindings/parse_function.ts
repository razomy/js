...
export function parseFunction(node: FunctionDeclaration): abstracts.translators.FunctionBinding {
  ...
  const doc = tsLang.ast.doc.tryParseJsDoc(node);
  const title = doc ? tsLang.ast.doc.parseTitle(doc, funcName) : '';
  const examples = doc ? tsLang.ast.doc.parseExamples(doc, funcName) : [];
  const returnDescription = doc ? tsLang.ast.doc.parseReturn(doc, funcName) : '';
  const complexity = doc ? tsLang.ast.doc.parseComplexity(doc, funcName) : {time: '', memory: ''};
  ...
  return {
    ...
    identifier: node.getNameNode()
      ? tsLang.ast.bindings.parseIdentifier(node.getNameNode()!)
    ...
    shapes,
    meta: {
      description: doc ? tsLang.ast.doc.parseFunctionDescription(doc) : '',
      title,
      performance: {
        history,
