...
function getContext(versionRuntimeDir: string) {
  ...
  const podmanExe = runtimesNode.getExePath(versionRuntimeDir, 'podman', 'podman.exe');
...
export const PODMAN_RUNTIME: runtimesNode.RuntimeProvider = {
  ...
  setup(versionWorkspaceDir, versionRuntimeDir) {
    ...
    try {
      ...
      const machineList = runtimesNode.execCmd(`${podmanExe} machine list`, versionWorkspaceDir, env);
      ...
      if (!machineList.includes('podman-machine-default')) {
        ...
        runtimesNode.execCmd(`${podmanExe} machine init`, versionWorkspaceDir, env);
      }
      ...
      if (!isRunning) {
        ...
        runtimesNode.execCmd(`${podmanExe} machine start`, versionWorkspaceDir, env);
      }
  ...
  run(versionWorkspaceDir, versionRuntimeDir, packageName, functionName, params) {
    ...
    return runNode.cli.spawnProcess(podmanExe.replace(/"/g, ''), args, versionWorkspaceDir, env);
  },
  ...
  install(packageName: string, versionWorkspaceDir: string, versionRuntimeDir: string) {
    ...
    runtimesNode.execCmd(`${podmanExe} pull ${image}`, versionWorkspaceDir, env);
  },

  // Аналог "remove" - удаление образа
  remove(packageName: string, versionWorkspaceDir: string, versionRuntimeDir: string) {
    const { podmanExe, env } = getContext(versionRuntimeDir);
    runtimesNode.execCmd(`${podmanExe} rmi ${packageName}`, versionWorkspaceDir, env);
  },
  ...
  list(versionWorkspaceDir: string, versionRuntimeDir: string): string[] {
    const { podmanExe, env } = getContext(versionRuntimeDir);
    try {
      const output = runtimesNode.execCmd(
